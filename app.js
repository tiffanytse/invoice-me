...
app.init = () => {
  ...
  app.addRow.click(function(){
    const newRow = `<tr class="item-row"><td class="js-delete-row"><span class="delete-icon" title="Remove row"><svg class="custom-icon-trash-o"><use xlink:href="#custom-icon-trash-o"></use></svg></span></td><td class="item-name"><textarea placeholder="Task/Item Name"></textarea></td><td class="description"> <textarea placeholder="Description"></textarea> </td><td> <input class="rate" type="number" value="0.00"></td><td> <input class="qty" type="number" value="0"> </td><td> <span class="line-total">$0</span> </td></tr>`;
  ...
  // $(".js-invoice-body").on("focus", ".rate",
  // function(){
  //   app.updateLineTotal;
  // });

  $('.rate, .qty, .select-tax').blur(app.updateLineTotal); // TODO: get to work with added dom elements
...
app.updateLineTotal = function() {
  ...
  let lineTotal = parseFloat(rate) * parseInt(qty);
...
app.updateTotals = () => {
  ...
  $('.line-total').each(function(i){
    ...
    if (!isNaN(lineTotal)) subTotal += parseFloat(lineTotal);
  });
